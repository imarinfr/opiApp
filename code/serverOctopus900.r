...
prepareZestRun <- function(eye, size, lum) {
  if(appParams$runType == "luminance") { # Luminance perimetry
    makeStimHelper <- function(x, y) { # returns a function of (db, n)
      ...
      body(ff) <- substitute({
        s <- list(x = x, y = y, eye = eye,
                  level = appParams$bg + appParams$maxlum * 10^(-db / 10),
                  size  = size,
                  color = appParams$color, duration = appParams$presTime,
                  responseWindow = appParams$respWindow)
    ...
    makeStimHelper <- function(x, y) { # returns a function of (db, n)
      ...
      body(ff) <- substitute({
        s <- list(x = x, y = y, eye = eye,
                  level = appParams$bg + lum,
                  size  = appParams$maxdiam * 10^(-db / 10),
                  color = appParams$color, duration = appParams$presTime,
                  responseWindow = appParams$respWindow)
        class(s) <- "opiStaticStimulus"
        return(s)
    ...
    minStim <- head(domain, 1) + domainOffset
    maxStim <- tail(domain, 1) - domainOffset
  } else stop("wrong test type")  return(list(domain = domain, prior_pmf = prior_pmf,
    ...
    makeStimHelper <- function() { # returns a function of (db, n)
      ...
      body(ff) <- substitute({
        s <- list(x = x, y = x, eye = eye,
                  level = appParams$maxlum * 10^(-db / 10),
                  size  = 0.43,
                  color = appParams$color, duration = appParams$presTime,
                  responseWindow = appParams$respWindow)
        class(s) <- "opiStaticStimulus"
  ...
  repeat{
    ...
    if(cmd == "opiInit") {
      ...
      if(!is.null(opiSetBackground(bgeye = eye, bglum = appParams$bg, bgcol = appParams$color,
                                   fixtype = "cross", fixsx = 2, fixsy = 2, fixlum = 3 * appParams$bg,
                                   fixcol = "green"))) {
    ...
    if(cmd == "opiBg") {
      # get eye from message
      eye <- ShinySender$pop()$message
      if (!is.null(opiSetBackground(eye = eye, lum = appParams$bg, color = appParams$color))) {
        ShinyReceiver$push("ERR", "opiSetBackground failed")
        break
      }
      cmd <- "opiWait"
